...
local function complete_step(step_name, success, error_message)
  ...
  if success then
    ...
    local message = string.format(
      ...
      M.current_step,
      M.total_steps,
      M.steps[M.current_step].description
    )
    ...
    -- エラーオブジェクトを作成して処理
    local error_obj = errors.create_error(errors.error_types.INTERNAL, message, {
      step = step_name,
...
function M.run_next_step(callback)
  ...
  elseif step.name == "storage" then
    ...
    complete_step(
      "storage",
      success,
      success and nil or "ストレージディレクトリの作成に失敗しました"
    )
    run_next_step_async(callback)
  ...
  elseif step.name == "api" then
    ...
    get_api().test_connection(function(success, data)
      if success then
        ...
        -- エラーオブジェクトを作成
        local error_obj =
          errors.create_error(errors.error_types.API, "Slack APIへの接続に失敗しました: " .. error_message, {
            step = "api",
            error_data = data,
          })
        ...
        if success then
          ...
          get_api().test_connection(function(reconnect_success, reconnect_data)
            if reconnect_success then
              notify(
                "再接続に成功しました - ワークスペース: " .. (reconnect_data.team or "Unknown"),
                vim.log.levels.INFO
              )
